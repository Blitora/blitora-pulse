...
export function getSignupMealTemplatePrompt(profile, userLocation) {
  ...
  const dietRules = [
    ...
    isVegan ? 'STRICTLY NO animal products — no eggs, dairy, honey' : null,
    noDairy ? 'STRICTLY NO dairy — no milk, curd, paneer, buttermilk, ghee, cheese' : null,
    isJain ? 'Jain — NO root vegetables (no onion, garlic, potato, carrot, beetroot, ginger)' : null,
...
CRITICAL DIETARY RULES — every single option must comply. Violation is not acceptable:
${dietRules.length > 0 ? dietRules.map(r => '- ' + r).join('\n') : '- No special restrictions'}

Condition-specific rules:
- Diabetic: low GI foods, no sugar/jaggery/honey, avoid white rice in large portions, no fruit juices
- High BP: very low sodium, no pickles/papad/processed foods
- Obese/Weight loss: calorie deficit, high fibre, no fried foods
- PCOD/Thyroid: anti-inflammatory, avoid excess soy
...
3. Every option must strictly follow ALL dietary rules above — no exceptions
4. Do NOT repeat same main ingredient across A, B, C in same slot
5. Practical foods — cookable in 15-20 minutes, locally available in ${userLocation || 'India'}
6. Condition rules: diabetic=low GI | BP=low sodium | obese=calorie deficit+high fibre
      ...
      "options": [
        {
          ...
          "totalFat": number,
          "items": [
            { "name": "food", "quantity": "2 idli / 150g", "calories": number, "protein": number, "carbs": number, "fat": number, "notes": "optional tip" }
          ]
        },
...
Slots to generate: breakfast, mid_morning, lunch, evening_snack, dinner${(mealPlanType||5) >= 6 ? ', post_dinner' : ''}`,
  }
}
