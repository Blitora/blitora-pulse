...
export default function MyPlan() {
  ...
  async function load() {
    ...
    const { data: prof } = await sb.from('profiles')
      .select('id,full_name,active_template_id,calorie_target,protein_target,carb_target,fat_target,water_target,health_conditions,dob,gender,goal_weight,current_weight,account_type')
      .eq('id', session.user.id)
      .single();
